...
(defn submit-jobs! [config-file-string]
  ...
  (let [{:keys [zookeeper-url] :as config} (read-config (io/resource config-file-string))
        env-config #spy/d (assoc (:env-config config)
                          :zookeeper/address zookeeper-url)
        peer-config #spy/d (assoc (:peer-config config)
                           :zookeeper/address zookeeper-url)
        env #spy/d (onyx.api/start-env env-config)
        peer-group #spy/d (onyx.api/start-peer-group peer-config)

        peer-count 6 ;; #spy/d (n-peers the-catalog the-workflow)
        v-peers #spy/d (onyx.api/start-peers peer-count peer-group)]
    ...
    (for [[the-workflow the-catalog] [[psc/workflow (psc/catalog zookeeper-url "scanner-command" "scanner")]
      ...
      (let [job {:workflow the-workflow
            ...
            {:keys [job-id task-ids] :as submitted-job} #spy/d (onyx.api/submit-job peer-config job)]
        submitted-job))))
